...
from h265ify import cmd_replace, cmd_encode
...
def test_cmd_replace_permanent_confirm(tmp_path: Path) -> None:
    ...
    with patch(
        "h265ify.commands.replace.find_replace_pairs",
        return_value=[ReplacePair(h265, orig)],
    ):
        with patch("h265ify.commands.replace.run_replace", return_value=(1, 0)):
            cmd_replace(args, console)
...
def test_cmd_encode_cpu() -> None:
    ...
    with patch("h265ify.commands.encode.find_video_files", return_value=[]):
        with patch("h265ify.commands.encode.probe_files", return_value=[]):
            with patch("h265ify.commands.encode.run_pipeline", return_value=([], False)):
                with pytest.raises(SystemExit) as e:
